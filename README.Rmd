...
The our lab has been collecting data on the exposure and symptom onset for Wuhan novel coronavirus (nCoV-2019) cases that have been confirmed outside of the Hubei province.
...
Two authors independently review the full text of each case report to ensure that data is correctly input.
...
ggplot(dat_sum, aes(y=factor(UID))) + 
    ...
    scale_x_continuous("Days since ER") +
    ylab("UID") +
    theme_bw() +
...
We estimate the incubation period using the \texttt{coarseDataTools} package based on the paper by Reich *et al*, 2009, using code originally written for determining the incubation period of Zika virus (Lessler *et al*, 2016).
...
plot(ncov_inc_fit_boot, main="overall results")
...
Finally, we challenge our assumption that unknown ELs can be assumed to be 2019 December 1 (Nextstrain estimates that it could have happened as early as September), by setting unknown ELs to 2018 December 1.
...
```{r year-dic-plots}
## plot the boot fit and table of intervals
plot(ncov_year_fit_boot, main="non-Mainland results")
...
all_est_plot <- ncov_inc_fit_asym@ests[-c(1,2), -4] %>% 
    as.data.frame() %>% 
    mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
           type="all") %>% 
    bind_rows(ncov_fever_fit_asym@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
                  # rename(est_fever=est, CIlow_fever=CIlow,
                  #        CIhigh_fever=CIhigh) %>% 
                  mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
                         type="fever")) %>% 
    bind_rows(ncov_foreign_fit_asym@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
                  # rename(est_foreign=est, CIlow_foreign=CIlow,
                  #        CIhigh_foreign=CIhigh) %>% 
                  mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
                         type="non-Mainland")) %>% 
    bind_rows(ncov_year_fit_asym@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
