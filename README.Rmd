...
We estimate the incubation period using the \texttt{coarseDataTools} package based on the paper by Reich *et al*, 2009.
...
all_est_plot <- ncov_inc_fit_boot@ests[-c(1,2), -4] %>% 
    as.data.frame() %>% 
    mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
           type="all") %>% 
    bind_rows(ncov_fever_fit_boot@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
                  # rename(est_fever=est, CIlow_fever=CIlow,
                  #        CIhigh_fever=CIhigh) %>% 
                  mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
                         type="fever")) %>% 
    bind_rows(ncov_foreign_fit_boot@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
                  # rename(est_foreign=est, CIlow_foreign=CIlow,
                  #        CIhigh_foreign=CIhigh) %>% 
                  mutate(qtile=c(2.5, 5, 25, 50, 75, 95, 97.5),
                         type="non-Mainland")) %>% 
    bind_rows(ncov_year_fit_boot@ests[-c(1,2), -4] %>% 
                  as.data.frame() %>% 
