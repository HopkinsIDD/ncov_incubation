...
```{r fit-dic-gamma, results='hide', cache=T, eval=F}
set.seed(1)

#fit it...use bootstrap interstions for now. MCMC better?
ncov_gam_fit_asym <- dic.fit(ncov_inc_dat,dist="G",
                             ptiles = c(0.025, 0.05, 0.25, 0.5, 0.75, 0.95, 0.975))
...
```{r fit-dic-weibull, results='hide', cache=T, eval=F}
set.seed(1)
...
```{r fit-dic-erlang, results='hide', cache=T, eval=F}
set.seed(1)
...
backer_comp <- ncov_inc_fit_asym@ests[-2, -4] %>% 
    as.data.frame() %>% 
    mutate(qtile=c("mean", 2.5, 5, 25, 50, 75, 95, 97.5),
           type="all") %>% 
    bind_rows(ncov_fever_fit_asym@ests[-2, -4] %>% 
