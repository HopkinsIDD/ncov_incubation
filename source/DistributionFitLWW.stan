...
parameters {
  ...
  real <lower=0> v_alpha;//alpha parameter for weibull distribution for viral shedding
  real <lower=0> v_sigma; //sigma prameter for weibull distribution for length of viral sheddig

  real <lower=0> s_alpha; //alpha parameter for weibull distribution for seroconversion
  real <lower=0> s_sigma; //sigma parameter for weibull distribution for seroconversion
...
model {
  ...
  for (i in 1:N) {
    ...
    increment_log_prob(log(weibull_cdf(LEN_VSP_LONG[i],v_alpha, v_sigma) - 
			   weibull_cdf(LEN_VSP_SHORT[i],v_alpha, v_sigma)));

    increment_log_prob(log(weibull_cdf(LEN_SCP_LONG[i], s_alpha, s_sigma) -
			   weibull_cdf(LEN_SCP_SHORT[i], s_alpha, s_sigma)));
  }
